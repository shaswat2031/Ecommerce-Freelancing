...
const AdminDashboard = () => {
  ...
  const handlePayoutAction = async (vendorId, transactionId, status, note = "") => {
    try {
      await client.put(`/admin/payouts/${vendorId}/${transactionId}`, { status, note });
      // Refresh payouts
  ...
  const fetchAdminVendorProducts = async (vendorId) => {
    try {
      const { data } = await client.get(`/admin/vendor-products?vendorId=${vendorId}&status=`);
  ...
  const sendAdminVendorMessage = async (vendorId, message) => {
    try {
      const { data } = await client.post(`/vendor-messages/admin/${vendorId}`, { message });
      return true;
    } catch (error) {
    ...
    const fetchInquiries = async () => {
      if (activeTab === "inquiries") {
        ...
        try {
          ...
          const { data } = await client.get(`/admin/returns?${params.toString()}`);
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-secondary/10">
              {filteredOrders.map((order) => {
                ...
                const uniqueVendors = Array.from(new Set(order.orderItems?.map(item => item.product?.vendor?.businessName || "In-House").filter(Boolean)));
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          {uniqueVendors.map((vendor, idx) => (
                            <span key={idx} className={`text-xs px-2 py-0.5 rounded-full w-fit font-bold ${vendor === 'In-House' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                              {vendor}
                            </span>
                          ))}
                        </div>
      ...
      <div className="space-y-6 animate-fade-in pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-surface p-6 rounded-sm border border-secondary/10">
            ...
            <p className="text-xs text-text-secondary">Gross Sales (Before Refunds)</p>
          </div>
          ...
          <div className="bg-surface p-6 rounded-sm shadow-sm border border-secondary/10 border-l-4 border-l-emerald-500">
            ...
            <p className="flex justify-between text-xs text-text-secondary mt-2">
              <span>Gross: {formatPrice(platformAnalytics?.revenue?.gross || 0)}</span>
              <span>Ref: -{formatPrice(platformAnalytics?.revenue?.refunds || 0)}</span>
            </p>
          </div>
                  ...
                  <tr key={vendor._id} className="hover:bg-secondary/5">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs px-2 py-1 rounded-full font-bold uppercase
                            ${vendor.subscription?.plan === 'enterprise' ? 'bg-purple-100 text-purple-700' :
                          vendor.subscription?.plan === 'professional' ? 'bg-blue-100 text-blue-700' :
                            'bg-gray-100 text-gray-700'}`}>
                        {vendor.subscription?.plan || 'Starter'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm font-bold text-primary">
                      {vendor.metrics?.totalProducts || vendor.inventory?.length || 0}
                    </td>
                    <td className="px-6 py-4">
                        ...
                        <p className="text-lg font-bold text-primary capitalize">
                          ...
                          {selectedVendor.subscription?.upcomingPlan && (
                            <span className="text-xs text-secondary block">
                              (Switches to {selectedVendor.subscription.upcomingPlan} on {new Date(selectedVendor.subscription.upcomingPlanDate).toLocaleDateString()})
                            </span>
                          )}
                        </p>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedVendor.complianceDocuments.map((doc, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-3 bg-secondary/5 rounded-sm"
                          >
                            <div className="flex items-center gap-3">
              ...
              {activeVendorModalTab === "products" && (
                <div className="space-y-4">
                  {adminVendorProducts.length === 0 ? (
                    <div className="text-center py-8 text-text-secondary">No products found for this vendor.</div>
                  ) : (
                    <div className="overflow-x-auto">
                        ...
                        <tbody className="divide-y divide-secondary/10">
                          {adminVendorProducts.map(prod => (
                            <tr key={prod._id} className="hover:bg-secondary/5">
                              <td className="p-3">
                                <div className="flex items-center gap-3">
                                  <img src={prod.images?.[0] || prod.image} alt="" className="w-10 h-10 object-contain bg-white rounded-sm border" />
                                  <div>
                                    <p className="font-medium text-primary">{prod.name}</p>
                                    <p className="text-xs text-text-secondary">{prod.category}</p>
                                  </div>
                                </div>
                              </td>
                              ...
                              <td className="p-3">
                                <span className={`text-xs px-2 py-1 rounded-full uppercase font-bold
                                                      ${prod.vendorStatus === 'approved' ? 'bg-green-100 text-green-700' :
                                    prod.vendorStatus === 'rejected' ? 'bg-red-100 text-red-700' :
                                      'bg-yellow-100 text-yellow-700'}`}>
                              ...
                              <td className="p-3">
                                <div className="flex gap-2">
                                  {prod.vendorStatus !== 'approved' && (
                                    <button
                                      onClick={async () => {
                                        await handleProductApproval(prod._id, 'approved');
                                        fetchAdminVendorProducts(selectedVendor._id);
                                      }}
                                  ...
                                  {prod.vendorStatus !== 'rejected' && (
                                    <button
                                      onClick={async () => {
                                        ...
                                        if (r) {
                                          await handleProductApproval(prod._id, 'rejected', r);
                                          fetchAdminVendorProducts(selectedVendor._id);
                                        }
                                      }}
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 bg-gray-100 text-gray-600 rounded hover:bg-gray-200" title="Delete">
                                    <Trash2 size={16} />
                                  </button>
                                </div>
                              </td>
                      ...
                      adminVendorMessages.map((msg) => (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <span className="uppercase font-bold mr-1 text-[8px] opacity-70">{msg.sender}</span> • {new Date(msg.createdAt).toLocaleString()}
                            </p>
                          </div>
                        </div>
                      ))
  ...
  const renderRefunds = () => (
    <div className="bg-surface rounded-sm shadow-sm border border-secondary/10 overflow-hidden animate-fade-in">
      <div className="p-6 border-b border-secondary/10">
        ...
        <p className="text-xs text-text-secondary mt-1">
          Track all refund activities for the last 45 days. Delivery charges are strictly non-refundable.
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-secondary/10">
            {adminRefundLogs.map((log) => (
              <tr key={log._id} className="hover:bg-secondary/5 transition-colors">
                <td className="px-6 py-4 font-mono text-xs">{log._id.slice(-6)}</td>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${log.initiatedBy === 'Admin' ? 'bg-purple-100 text-purple-700' :
                    log.initiatedBy === 'Vendor' ? 'bg-blue-100 text-blue-700' :
                      'bg-yellow-100 text-yellow-700'
                    }`}>
                    {log.initiatedBy}
                  </span>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-secondary/10">
                {payouts.length === 0 ? (
                  ...
                  payouts.map((payout) => (
                    <tr key={payout._id} className="hover:bg-secondary/5 transition-colors">
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        <button
                          onClick={() => {
                            if (window.confirm("Approve this payout?")) {
                              handlePayoutAction(payout.vendorId, payout._id, "completed");
                            }
                          }}
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (note) {
                              handlePayoutAction(payout.vendorId, payout._id, "rejected", note);
                            }
                          }}
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            { label: "Requested", value: returnsCounts.requested || 0, color: "yellow" },
            { label: "Approved", value: returnsCounts.approved || 0, color: "green" },
            { label: "Rejected", value: returnsCounts.rejected || 0, color: "red" },
            { label: "Returned", value: returnsCounts.returned || 0, color: "blue" },
            { label: "Refunded", value: returnsCounts.refunded || 0, color: "purple" },
          ].map((stat) => (
            <div
              key={stat.label}
              onClick={() => setReturnsFilter(stat.label.toLowerCase())}
              className={`bg-surface p-4 rounded-sm shadow-sm border border-secondary/10 cursor-pointer hover:shadow-md transition-shadow ${returnsFilter === stat.label.toLowerCase() ? `ring-2 ring-${stat.color}-400` : ""
                }`}
            >
                ...
                {returnsLoading ? (
                  ...
                  adminReturns.map((ret) => (
                    <tr key={ret._id} className="hover:bg-secondary/5 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-primary">{ret.vendorName}</div>
                        <div className="text-xs text-text-secondary">{ret.vendorEmail}</div>
                      </td>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-surface rounded-sm p-8 max-w-lg w-full shadow-2xl border border-secondary/10 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4 mb-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs uppercase text-text-secondary mb-1">Vendor</p>
                    <p className="text-sm font-medium text-primary">{selectedReturn.vendorName}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase text-text-secondary mb-1">Amount</p>
                    <p className="text-sm font-bold text-primary">{formatPrice(selectedReturn.subtotal)}</p>
                  </div>
                </div>

                <div>
                  <p className="text-xs uppercase text-text-secondary mb-1">Product(s)</p>
                  <p className="text-sm text-primary">{selectedReturn.productName}</p>
                </div>

                <div>
                  <p className="text-xs uppercase text-text-secondary mb-1">Return Reason</p>
                ...
                {selectedReturn.customerComment && (
                  <div>
                    <p className="text-xs uppercase text-text-secondary mb-1">Customer Comment</p>
                ...
                <div>
                  <p className="text-xs uppercase text-text-secondary mb-1">Current Status</p>
                  {getStatusBadge(selectedReturn.status)}
                </div>
              </div>

              <div className="border-t border-secondary/10 pt-6">
                <p className="text-xs uppercase text-text-secondary mb-3">Update Status</p>
                <div className="flex flex-wrap gap-2">
                  {["Approved", "Rejected", "Returned", "Refunded"].map((status) => (
                    <button
                      key={status}
                      onClick={() => {
                        if (window.confirm(`Update status to "${status}"?`)) {
                          handleUpdateReturnStatus(selectedReturn._id, status);
                        }
                      }}
